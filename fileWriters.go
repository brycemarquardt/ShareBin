...
func MultipleFileWriter(files []*multipart.FileHeader, path string, aesKey []byte, callback func()) {
    ...
    if err != nil {
        fmt.Println("ShareBin: Failed to create multiple file output:", err)
        return
    }
    defer outFile.Close()
    ...
    for _, fileHeader := range files {
        ...
        if err != nil {
            fmt.Println("ShareBin: Failed to open multiple file:", err)
            return
        }
        defer file.Close()
        ...
        if err != nil {
            fmt.Println("ShareBin: Failed to create zip entry:", err)
            return
        }
        ...
        for {
            ...
            if err != nil && err != io.EOF {
                fmt.Println("ShareBin: Error reading multiple file chunk:", err)
                return
            }
            if n == 0 {
                break
            ...
            if err != nil {
                fmt.Println("ShareBin: Failed to write to zip:", err)
                return
            }
    ...
    if aesKey != nil {
        ...
        if err != nil {
            fmt.Println("ShareBin: Failed to encrypt multiple file ZIP:", err)
        }
    }
...
func SingleFileWriter(files []*multipart.FileHeader, path string, aesKey []byte, callback func()) {
    outFile, err := os.Create(path)
    if err != nil {
        fmt.Println("ShareBin: Failed to create single file output:", err)
        return
    }
    defer outFile.Close()
    ...
    if err != nil {
        fmt.Println("ShareBin: Failed to open single file:", err)
        return
    }
    defer file.Close()
    ...
    for {
        n, err := file.Read(buffer)
        if err != nil && err != io.EOF {
            fmt.Println("ShareBin: Error reading single file chunk:", err)
            return
        }
        if n == 0 {
            break
